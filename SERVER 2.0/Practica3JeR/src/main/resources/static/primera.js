class primera extends Phaser.Scene {
    ...
    preload(){
      ...
      this.load.image("borrar", "resources/si.png");  
    }
    create (){
        this.clickButton = this.add.image(550, 475, 'jugar' )
         ...
         this.clickButton = this.add.image(550, 200, 'borrar')
	         .setInteractive()
	         .on('pointerdown', () => jugador.id = $("#borrar-id").val())
	         .on('pointerdown', () => console.log(jugador.id))
	         .on('pointerdown', () => borrarJugadores(jugador.id))
	         this.clickButton.setScale(0.5);
